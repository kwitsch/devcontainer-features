...
| `installIdeExtension` | `true` | Final step of `postCreate.sh`: probes well-known VS Code Server CLI paths under `$TARGET_HOME` (`code`, `code-insiders`, `cursor`) and runs `<cli> --install-extension anthropic.claude-code --force` as the target user. Soft-skips when no CLI is available (Codespaces prebuilds, JetBrains, headless containers). |
