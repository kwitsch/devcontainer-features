...
> **Host requirement:** the Feature reads host OAuth tokens via bind mounts of `$HOME/.claude.json` and `$HOME/.claude/` (the lifecycle scripts only read from them — the Feature manifest cannot declare them `readonly` at the OS level, see [`NOTES.md`](src/claude-code/NOTES.md)). **macOS hosts are not supported** (Claude Code stores tokens in the Keychain, not on disk). See [`NOTES.md`](src/claude-code/NOTES.md) for full platform compatibility and edge-case behavior.
