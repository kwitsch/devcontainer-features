...
| installIdeExtension | When true (default) and a VS Code-family CLI (`code`/`code-insiders`/`cursor`) is on the target user's PATH at postCreate time, install the `anthropic.claude-code` extension via `<cli> --install-extension anthropic.claude-code --force`. Silent skip when no CLI is found (Codespaces prebuilds, JetBrains hosts, headless containers). | boolean | true |
...
**`installIdeExtension`** — final step of `postCreate.sh`. Probes the well-known dev-container CLI paths (`~/.vscode-server/bin/*/bin/remote-cli/code`, the `code-insiders` analogue, and `~/.cursor-server/bin/*/bin/remote-cli/cursor`) under the target user's home, with a `bash -lc 'command -v …'` fallback for non-standard setups. When one is found, runs `<cli> --install-extension anthropic.claude-code --force` as the target user. This is the same command the Claude Code binary itself invokes when `/ide` is used without the extension installed (verified by inspecting the binary). Soft-skips when no CLI is available, so Codespaces prebuilds, JetBrains hosts, and headless containers stay quiet.
